...
Wp3=6000
Ws=600; 
Ws3=7000;               %Stop band edge = 600Hz    ，，阻带上限7000
...
RES1=windowed_filter_transfer_function(hd_win,N);    %tep 5: get the frequency response of the windowed unit impulse response
                                                    %得到加窗单位脉冲响应的频率响应
%=============================================================================================================================
HD3=lowpass_transfer_function(DC,Wp_n3,N);    %Step 1: define ideal lowpass filter response 定义理想的低通滤波器响应
hd3=unit_impulse_response(HD3,N);             %Step 2: convert filter response to unit impulse response 将滤波器响应转换为单位脉冲响应h（n）
[win3 Nwin3]=rectangular_window(fs,TW3);       %Step 3: generate rectangular window function 生成矩形窗函数
hd_win3=add_window(hd3,win3,DC,Nwin3,N)         %Step 4: apply window function to unit impulse response 将窗函数应用于单位脉冲响应
RES3=windowed_filter_transfer_function(hd_win3,N);    %tep 5: get the frequency response of the windowed unit impulse response
                                                    %得到加窗单位脉冲响应的频率响应
RES=RES3-RES1
